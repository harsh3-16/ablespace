...
export class CategoryScraperService {
    ...
    async scrape(url: string, navigationId?: string): Promise<number> {
        ...
        const crawler = new PlaywrightCrawler({
            ...
            requestHandler: async ({ page, request }) => {
                ...
                try {
                    await page.waitForSelector('.ais-InfiniteHits-item, .grid__item', { timeout: 5000 });
                ...
                const categoryItems: ScrapedCategoryItem[] = await page.evaluate(() => {
                    ...
                    const results: Array<{ title: string; href: string; imageUrl: string | undefined }> = [];
    ...
    private deduplicateCategories(items: Partial<Category>[]): Partial<Category>[] {
