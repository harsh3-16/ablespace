    ...
    async scrapeProductList(url: string, categoryId?: string): Promise<number> {
        ...
        const crawler = new PlaywrightCrawler({
            ...
            requestHandler: async ({ page, request }) => {
                ...
                try {
                    await page.waitForSelector('.ais-InfiniteHits-item, .grid__item, .product-card', { timeout: 5000 });
                ...
                const productItems: ScrapedProduct[] = await page.evaluate(() => {
                    ...
                    for (const el of elements) {
                        ...
                        const titleEl = el.querySelector('.product-card.truncate-title, .product-card, h2, h3');
    ...
    async scrapeProductDetail(url: string): Promise<number> {
        ...
        const crawler = new PlaywrightCrawler({
            ...
            requestHandler: async ({ page, request }) => {
                ...
                try {
                    const additionalInfoBtn = await page.$('button.accordion-head:has-text("Additional")');
                ...
                scrapedDetails = await page.evaluate((): ScrapedDetail => {
                    ...
                    const author =
                        ...
                        document.querySelector('a[href*="/collections/author-"]')?.textContent?.trim() ||
                        '';

                    // Description - from JSON-LD or Summary accordion
                    let description = jsonLdData?.description || '';
                    if (!description) {
                        const summaryBtn = Array.from(document.querySelectorAll('button.accordion-head'))
                            .find(b => b.textContent?.toLowerCase().includes('summary'));
                        if (summaryBtn?.nextElementSibling) {
                            description = summaryBtn.nextElementSibling.textContent?.trim() || '';
                        }
                    }
                    ...
                    // ISBN
                    const isbn = jsonLdData?.isbn || document.querySelector('#info-isbn13')?.textContent?.trim();
                    if (isbn) specs['isbn'] = isbn;

                    // Format/Binding
                    const format = jsonLdData?.bookFormat || document.querySelector('#info-binding-type')?.textContent?.trim();
                    if (format) specs['format'] = format;

                    // Publisher
                    const publisher = jsonLdData?.publisher?.name || document.querySelector('#info-publisher')?.textContent?.trim();
                    if (publisher) specs['publisher'] = publisher;

                    // Pages
                    const pages = document.querySelector('#info-pages')?.textContent?.trim();
                    if (pages) specs['pages'] = pages;

                    // Publication date
                    const pubDate = jsonLdData?.datePublished || document.querySelector('#info-publication-date')?.textContent?.trim();
                    if (pubDate) specs['publicationDate'] = pubDate;

                    return { title, description, price, author, imageUrl, specs };
                });

                this.logger.log(`Scraped: "${scrapedDetails?.title}" by ${scrapedDetails?.author}`);
                this.logger.log(`  Price: ${scrapedDetails?.price}, Image: ${scrapedDetails?.imageUrl ? 'Yes' : 'No'}`);
        ...
        try {
            ...
            const priceNum = details.price ? parseFloat(details.price.replace(/[^0-9.]/g, '')) : undefined;
            ...
            const detailData = {
                ...
                specs: details.specs,
                isbn: details.specs?.isbn,
                format: details.specs?.format,
                publisher: details.specs?.publisher,
                pages: details.specs?.pages ? parseInt(details.specs.pages, 10) : undefined,
                publicationDate: details.specs?.publicationDate,
            };
