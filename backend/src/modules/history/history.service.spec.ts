...
describe('HistoryService', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                HistoryService,
                { provide: getRepositoryToken(ViewHistory), useValue: mockHistoryRepository },
            ],
        }).compile();
    ...
    describe('addPathToHistory', () => {
        it('should add path to existing history', async () => {
            const existingHistory = {
                ...mockHistory,
                pathHistory: [{ path: '/', title: 'Home', timestamp: '2024-01-01T00:00:00Z' }],
            };
            ...
            historyRepo.save.mockResolvedValue({
                ...
                pathHistory: [
                    { path: '/', title: 'Home', timestamp: '2024-01-01T00:00:00Z' },
                    { path: '/products', title: 'Products', timestamp: expect.any(String) },
                ],
                lastPath: '/products',
            } as ViewHistory);

            const result = await service.addPathToHistory('session-123', '/products', 'Products');
        ...
        it('should create new history for new session', async () => {
            ...
            historyRepo.create.mockReturnValue({
                ...
                pathHistory: [{ path: '/', title: 'Home', timestamp: expect.any(String) }],
                lastPath: '/',
            } as ViewHistory);
            historyRepo.save.mockResolvedValue({
                id: 'new-id',
        ...
        it('should limit history to 50 entries', async () => {
            const fullHistory = {
                ...mockHistory,
                pathHistory: Array(50).fill({ path: '/test', title: 'Test', timestamp: '2024-01-01' }),
            };
            historyRepo.findOne.mockResolvedValue(fullHistory as ViewHistory);
            historyRepo.save.mockImplementation((h) => Promise.resolve(h as ViewHistory));
    ...
    describe('clearHistory', () => {
        it('should delete history by session ID', async () => {
            ...
            expect(historyRepo.delete).toHaveBeenCalledWith({ sessionId: 'session-123' });
        });
    });
});
